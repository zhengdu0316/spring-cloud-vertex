package org.vertex.es.controller;
...
import org.vertex.es.entity.Book;
import org.vertex.es.entity.HouseholderInfo;
import org.vertex.es.entity.param.ESQuery;
import org.vertex.es.entity.vo.ESReturnVO;
...
/**
 * @author zhengdu
 * @date 2019/5/18 11:06
 */
@RestController
@RequestMapping("/ap1/v1/es/householder")
public class ElasticsearchUniversalController {
...
//    }
    ...
    public ESReturnVO search(@RequestBody ESQuery esQuery) throws Exception{
        Map<String, String> paramMap = new HashMap<String, String>();
        paramMap.put("q", "area_code:330106*");
        paramMap.put("from", "0");
        paramMap.put("size", "500");
        paramMap.put("pretty", "true");
        String endpoint = "/"+esQuery.getIndex()+"/"+esQuery.getType()+"/"+"_search";
        Response response = client.performRequest("GET",   endpoint, paramMap);
        String responseBody = EntityUtils.toString(response.getEntity());
        com.alibaba.fastjson.JSONObject jsonObject = com.alibaba.fastjson.JSONObject.parseObject(responseBody);
//        ESReturnVO<HouseholderInfo> esReturnVO = new ESReturnVO<HouseholderInfo>();
        ESReturnVO esReturnVO = jsonObject.toJavaObject(ESReturnVO.class);
    ...
    @PostMapping(value = "/search2",consumes = MediaType.APPLICATION_JSON_UTF8_VALUE)
    public ESReturnVO search2(@RequestBody ESQuery esQuery) throws Exception{
        Map<String, String> paramMap = new HashMap<String, String>();
//        paramMap.put("q", "area_code:330*");
//        paramMap.put("pretty", "true");
        String endpoint = "/"+esQuery.getIndex()+"/"+esQuery.getType()+"/"+"_search";
...
//        HttpEntity entity = new NStringEntity("{\n" +
//                "  \"query\": {\n" +
//                "    \"match\": {\"area_code\":\"330106079001\"}\n" +
//                "  }\n" +
//                "}", ContentType.APPLICATION_JSON);
        Response response = client.performRequest("GET",   endpoint, Collections.emptyMap(),entity);
...
//        latch.await();
    ...
    public ResponseEntity<String> add2(@RequestBody Book book) throws IOException {
        // 构造HTTP请求，第一个参数是请求方法，第二个参数是服务器的端点，host默认是http://localhost:9200，
        // endpoint直接指定为index/type的形式
        Request request = new Request("POST", new StringBuilder("/"+book.getIndex()+"/"+book.getType()+"/").
                append(book.getId()).toString());
        // 设置其他一些参数比如美化json
        request.addParameter("pretty", "true");
     ...
     * @throws IOException
     */
    @RequestMapping(value = "/{id}/{index}/{type}", method = RequestMethod.GET)
    public ResponseEntity<String> getBookById(@PathVariable("id") String id,@PathVariable("index") String index,@PathVariable("type") String type) {
    ...
    public ResponseEntity<String> getAll() {
        Request request = new Request("GET", new StringBuilder("/hzrhcms/base_householder_info/").toString());
        // 添加json返回优化
        request.addParameter("pretty", "true");
        Response response = null;
        String responseBody = null;
